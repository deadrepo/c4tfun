...
def get_latest_token():
    ...
    try:
        print(f"Fetching latest token from {url}...")
        response = requests.get(url, verify=False)  # Bypassing SSL verification
        print(f"Response Status Code: {response.status_code}")
        print(f"Response Content: {response.text}")  # For debugging purposes

        if response.status_code == 200:
            tokens_data = response.json()
            latest_token = tokens_data["body"][0]["pool"]["y"]["asset"]
            print(f"Latest Token: {latest_token}")
            return latest_token
        else:
...
def get_token_details(policy_id):
    ...
    response = requests.get(details_url, verify=False)  # Bypassing SSL verification
...
def download_image(image_url, file_path):
    if image_url:
        response = requests.get(image_url, verify=False)  # Bypassing SSL verification
        if response.status_code == 200:
